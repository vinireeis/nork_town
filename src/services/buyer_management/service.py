...
class ClientService:
    ...
    async def linking_car_to_owner(
        cls, payload_validated: CarValidator, client_id: int
    ) -> bool:
        ...
        new_car_linking = CarModel(color=color, model=model, client_id=client_id)
    ...
    async def get_all_cars(cls):
        ...
        result = {"cars": [car.as_dict() for car in cars]}
        return result
